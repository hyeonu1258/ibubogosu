...
function registUser(req, res) {
    registQuery = 'insert into user(user_id, sns_token, prof_image_url, type, height, weight) values(?,?,?,?,?,?)';
    inserts = [req.body.user_id, req.body.sns_token, req.body.prof_image_url, req.body.type, req.body.height, req.body.weight];
    pool.getConnection(function(err, conn) {
        if (err) {
            ...
            async.series([
                ...
                function(callback) {
                    conn.query(registQuery, inserts, function(err, rows) {
                        if (err)  callback(msg(1, 'query err: ' + err, []));
